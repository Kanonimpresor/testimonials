...
// [PLUGINS]/testimonials/languages/[LANGUAGE]/[LANGUAGE]_global.php
e107::lan('testimonials', 'global', true);

// [PLUGINS]/testimonials/languages/[LANGUAGE]/[LANGUAGE]_admin.php
e107::lan('testimonials', true, true);

// Fallback: ensure LAN constants are defined
if(!defined('LAN_PLUGIN_TESTIMONIALS_NAME'))
{
	define('LAN_PLUGIN_TESTIMONIALS_NAME', 'Testimonials');
}
...
class testimonials_admin_items_ui extends e_admin_ui
{
	...
	protected $fields = array(
		...
		'tm_name'  => array(
			...
			'thclass'  => 'left',
			'readonly' => false,
			'writeParms' => array('size' => 'xlarge', 'placeholder' => LAN_TESTIMONIALS_ADMIN_18),
			'validate' => true,
			'help'     => LAN_TESTIMONIALS_ADMIN_19,
		),
		'tm_url'   => array(
	...
	{
		if(empty($new_data['tm_order']))
		{
			$c = e107::getDb()->count('testimonials');
			$new_data['tm_order'] = $c ? $c : 0;
		}

		if(empty($new_data['tm_datestamp']))
		{
			$new_data['tm_datestamp'] = time();
		}

		// Ensure tm_name has the UID.Name format
		if(!empty($new_data['tm_name']) && strpos($new_data['tm_name'], '.') === false)
		{
			$new_data['tm_name'] = '0.' . $new_data['tm_name'];
		}
