...
class testimonials_shortcodes extends e_shortcode
{
	...
	function sc_testimonials_indicators()
	{
		...
		{
			$slideLabel = defined('LAN_TESTIMONIALS_17') ? LAN_TESTIMONIALS_17 : 'Slide';
			for($i = 0; $i < (int) $this->var['count']; $i++)
			{
				$active = ($i === 0) ? ' class="active" aria-current="true"' : '';
				$indicators[] = '<button type="button" data-bs-target="#testimonials-carousel" data-bs-slide-to="' . $i . '"' . $active . ' aria-label="' . $slideLabel . ' ' . ($i + 1) . '"></button>';
			}
		}
		return '<div class="carousel-indicators">' . implode('', $indicators) . '</div>';
	}
	...
	function sc_testimonials_message()
	{
		...
		else
		{
			...
			// Only add truncated span if there is actually remaining text.
			if(!empty($remainder))
			{
